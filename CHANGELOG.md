...
- **Fixed "Saved vs retail" subtracting two different currencies**: LEGO retail prices only exist in the region currency (USD, GBP, CAD, EUR), so if you buy in anything else, "Saved vs retail" was doing things like 149.99 USD minus 600 DKK and showing the result as if it meant something. New `BK_SIDECAR_MSRP_RATE` setting takes a fixed exchange rate you look up once (e.g. `6.47` for USD to DKK) and converts MSRP into your purchase currency, on the set page and in the Statistics totals. Leave it empty and nothing converts, but the savings figure now carries a warning icon so at least it is obvious the two numbers do not match
- **`BK_SIDECAR_RETAIL_REGION` takes a fallback list**: Brickset only has non-US retail prices for reasonably modern sets, so picking `DE` because European pricing converts closer to yours meant every older set lost its MSRP entirely. The setting now accepts `DE,US` and uses the first region that actually has a price. Since those two regions are not the same currency, `BK_SIDECAR_MSRP_RATE` also takes one rate per currency (`EUR:7.45,USD:6.47`); a currency you leave out stays unconverted rather than being converted at the wrong rate, and the Statistics totals only count sets that landed in your currency
- **Price comparison shows where MSRP came from**: The "Retail (MSRP)" row got an info icon like the "Worth now" rows have. Hovering it tells you which LEGO.com region the price was read from and, if it was converted, the original amount and the rate used
